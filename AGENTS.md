...
| DDD 领域驱动设计 | `01-.../10-计算机设计思想与架构工程能力/DDD领域驱动设计/` | 11 | 总览(定位:复杂业务系统建模方法论,战略层切块+战术层建模,与软件架构能力体系培养/软件工程上的设计思想分工)→核心概念(领域-子领域分类投资策略/限界上下文四识别信号/统一语言五落地载体/关系链)→战略设计(六主三辅映射模式/防腐层vs适配器/决策树/七常见错误)→事件风暴(三风格/主持人手法/材料清单/六失败模式/LLM辅助边界ICSA实验)→战术建模上(实体-值对象-聚合/决策序列/值对象案例集/聚合拆分案例)→战术建模下(领域服务vs应用服务/仓储契约/工厂/领域事件/事件发布链路/决策速查)→微服务落地(分布式单体三探针/映射规则/模块化单体回潮/迁移绞杀者模式/十问自检)→Java与Spring实现(包结构/聚合代码形态/事务锚点/JPA纪律/测试金字塔/Round-trip工具化)→实战案例(电商订单端到端:事件风暴→上下文映射→战术建模→服务化决策→其他四上下文速览)→DDD×AI(LLM辅助事件风暴能力边界/Architecture Coding神经符号双核/DDD-Enforcer/Agent上下文隔离/提示词纪律)→生产与面试(12项清单/故障排查表/12面试题/四答题模板/速记)（2026-08：DDD Europe 2026/ICSA 2026/Explore DDD 2026基准；LLM辅助建模会系统性遗漏关键元素；分布式单体仍是头号反模式；模块化单体+右规模服务为推荐起点） |
| MVC | `01-.../10-计算机设计思想与架构工程能力/MVC/` | 11 | 总览(定位:架构史上生命力最强模式,1979-2026全景,后端MVC+前端MVVM双线,与DDD/软件架构能力体系培养分工)→起源与核心思想(Smalltalk/关注点分离/被动vs主动模型/模式组合/为什么是Controller/三次危机)→三组件深度解析(职责边界/薄控制器三边界/贫血模型/依赖方向/状态-行为配对/Spring映射双层结构)→家族对比(MVP-MVVM-Flux/MVCPVM等价性悖论2026/三问清单/可测试性/React Native验证)→前端困境与演进(上帝控制器四困境/Backbone复盘/MVVM-组件化-微前端三步/后端映射启示)→Spring MVC实践(薄控制器纪律/值对象绑定/ProblemDetail/批量赋值防护/请求生命周期七关卡/MockMvc测试)→Spring7新特性(@ApiVersion四策略/语义版本/ProblemDetail标准化/PathPatternParser/Jackson3/虚拟线程/升级三问)→现代前端替代(MVVM数据绑定三层/ViewModel三纪律/Vue响应式vs React渲染循环/依赖追踪深潜/四条误用)→状态管理演进(Flux单向流/Redux三原则/原子化Zustand/服务端状态失效机制/五类分类管理/三教训)→与周边架构关系(分层-六边形-DDD-微服务四轴/MVC与REST辨析/前后端分离/康威定律团队视角)→生产与面试(10项清单/8故障排查/12面试题/四答题模板/评审演练/速记)（2026-08：Spring Framework 7.0/Boot 4/Jackson 3/Vue 3基准；MVCPVM五角色显式共存提案；2026格局=后端MVC+前端MVVM/组件化+Flux兜底复杂状态） |
| 软件工程上的设计思想 | `01-.../10-计算机设计思想与架构工程能力/软件工程上的设计思想/` | 11 | 总览(定位:28条工程高频设计思想,非23种设计模式而是思想层,面向后端/系统工程/架构面试,与MVC/DDD分工:思想是判断依据模式是落地模板)→SOLID五大原则(SRP/OCP/LSP/ISP/DIP各自定义-工程体现-反例-面试话术/工具链ArchUnit/2026实证:工具箱非清单)→类与模块设计(迪米特法则/高内聚低耦合/正交设计/订单模块重构案例/与SOLID衔接)→工程约束四原则(KISS/YAGNI/DRY/SoC各自边界与冲突排序/违规识别静态信号)→健壮性设计(防御式编程vs吞异常/Fail-Fast/容错五件套降级熔断限流重试隔离/分层实施/量化验收)→幂等设计专项(Token-业务Key-版本号三方案/幂等键四要素/状态机回滚边界陷阱-操作版本号/Redis Lua框架/与分布式事务Saga-TCC协同)→状态机思想专项(FSM三要素/迁移表vs if-else/不可变状态/CAS并发/泛型触发器/事件溯源/幂等内置/Java完整代码)→配置与演进(配置驱动-规则参数分离/分层思想/约定大于配置/向后兼容-版本治理/各三问清单)→系统韧性(最小权限/可观测性三支柱落地配置/缓存四大问题击穿穿透雪崩一致性/备份冗余3-2-1与RPO-RTO)→并发与规模(分治思想分片键设计/推拉思想Kafka消费模型/悲观乐观与隔离级别联动/库存系统实战案例)→生产与面试(12项落地清单/12故障排查/28条速记表/12面试题/四答题模板)（2026-08：SOLID 2026学术实证/幂等边界陷阱华为云/Redis Lua幂等框架阿里云/状态机重构实践基准；面试优先级:幂等-状态机-缓存-容错为第一梯队） |
...
| LoRA 深度剖析 | `03-AI.../09-模型微调与多模态/LoRA深度剖析/` | 11 | 总览(定位:参数高效微调事实标准,与02-LoRA微调原理与实战入门版/大模型微调知识体系工程版分工)→原理与数学(ΔW=BA/秩边界/缩放等价/初始化/B-A分工/显存账目/局限)→为什么有效(内在低秩假设/子空间重叠实验/线性化视角/与全参定量对照/遗忘)→超参数工程(α-r比例铁律/模块选择/任务-超参映射/扫描方法/dropout定位/数据质量)→QLoRA(NF4分位量化/双重量化/分页优化器/量化-适配器交互/FSDP/版本线)→变体谱系(五条路线/RLVR 2026实证/PiSSA谱坍缩/失败模式/组合/选型决策树)→PEFT训练实战(SFT-DPO-GRPO/端到端模板/监控读数/多适配器/续训/报错/资产规范)→合并与多LoRA服务(merge细节/S-LoRA-Punica内核/vLLM配置/容量/监控)→多模态与扩散(VLM语言侧/LLaVA流程/DreamBooth+LoRA/UNet挂载/Civitai生态)→安全与前沿(对齐失效机理/后门/供应链/CSULoRA/秩分配三篇/评估清单)→生产与面试(清单/四案例/12面试+加分题/速记)（2026-08：PEFT 0.19.1/bnb 0.49.2/transformers 5.x/vLLM 0.14-0.15基准；RLVR实证DoRA 46.6%>全参44.9%>LoRA 42.5%；CSULoRA攻击成功率60.58%→1.73%） |
...
| 内存墙 | `01-.../计算机硬件方向系统学习清单/内存墙/` | 11 | 总览→起源(Wulf&McKee 1995/Dennard终结/算力6万x vs带宽100x)→层次结构(时延阶梯1ns-100ns-20µs/缓存行/缺页悬崖)→带宽墙与容量墙(Roofline/算术强度1 vs 295/离散容量墙)→处理器侧缓冲(缓存预取乱序多线程/3C缺失/TLB/MESI)→内存技术演进(DDR→HBM3e 1.2TB/s→HBM4 2.5-2.8TB/s 2048bit/供应危机+50%)→AI时代失衡(训练计算受限/推理内存受限/KV Cache 70B@128K 150GB/MoE并行)→PIM近内存计算(HBM-PIM AiM 2x+70%/StreamDQ近内存反量化/zHBM 4-8x/光子)→CXL内存池化(3.0机架池化/4.0商用验证/NVLink分工/KV分层/Penguin 11TB)→软件侧突破(分块/SoA/预取距离/NUMA/大页63%)→生产实践(四级内存架构/六周闭环/技术雷达/7面试)（2026-08：HBM4 2.0-2.8TB/s、SK海力士57-62%、zHBM、CXL 4.0、Meta Vistara省25%服务器、SOCAMM2） |
