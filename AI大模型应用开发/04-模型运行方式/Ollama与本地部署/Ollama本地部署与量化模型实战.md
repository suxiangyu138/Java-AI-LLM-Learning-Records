...
> **核心认知**：Ollama 让本地部署大模型像 `docker pull` 一样简单。结合量化技术，7B 模型只需 4GB 显存即可运行。
> **前置阅读**：`AI模型服务化部署全解析.md`
...
ollama run deepseek-coder    # DeepSeek Coder（~4.5GB，代码能力强）
...
client = OpenAI(
    ...
    api_key="ollama"  # 任意值，本地不需要真实 key
)
...
代价：精度的微小损失（通常 < 1% 质量下降）
```

### 2.2 GGUF 量化格式

Ollama 使用 GGUF 格式，常见的量化级别：

| 量化级别 | 大小（7B为例） | 质量 | 适用场景 |
|----------|---------------|------|----------|
| `q2_K` | ~2.9 GB | 明显下降 | 仅用于测试 |
| `q3_K_M` | ~3.5 GB | 可接受 | 资源极度受限 |
| `q4_K_M` | ~4.4 GB | 几乎无损失 | **推荐日常使用** |
| `q5_K_M` | ~5.2 GB | 极佳 | 高质量要求 |
| `q8_0` | ~7.7 GB | 接近无损 | 生产环境 |
| `f16` | ~14 GB | 原始精度 | 不在乎显存时 |
...
# 常见量化版本
ollama run qwen2:7b-q4_K_M    # 推荐日常
...
### 3.3 进阶：从 GGUF 文件创建模型

```bash
# 1. 下载 GGUF 文件（从 HuggingFace 或 ModelScope）
# 如：wget https://huggingface.co/Qwen/Qwen2-7B-Instruct-GGUF/resolve/main/qwen2-7b-instruct-q4_k_m.gguf
  ...
  ollama:
    ...
    command:
      - |
        ollama serve &
        sleep 3
        ollama pull qwen2:7b-q4_K_M  # 自动拉取模型
        tail -f /dev/null
...
# 2. nvidia-container-toolkit 已安装（Docker 环境）
```
...
# 问题4：GPU 未识别（Docker 环境）
docker run --gpus all ollama/ollama  # 加 --gpus all 参数
...
- [ ] 模型文件是否完整？显存是否足够加载？
- [ ] 如果是 Docker 部署，是否加了 `--gpus all`？
- [ ] Modelfile 的 TEMPLATE 格式是否匹配模型？（ChatML / Llama3 / DeepSeek 格式不同）
- [ ] 启动参数 `num_ctx` 设置是否合理？（默认 2048，大文档场景建议 8192+）
