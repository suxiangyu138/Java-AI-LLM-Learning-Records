...
> **核心摘要**：Zero-Shot、Few-Shot 和 Chain-of-Thought（CoT）是 Prompt Engineering 的三大核心范式。本文通过完整的 Python 代码示例，深入讲解每种范式的适用场景、实现方法和工程化落地技巧，包括动态示例选择、Self-Consistency 以及 Prompt 版本管理。
...
1. [三种核心范式对比](#1-三种核心范式对比)
2. [Zero-Shot 提示](#2-zero-shot-提示)
3. [Few-Shot 提示](#3-few-shot-提示)
4. [Chain-of-Thought（思维链）](#4-chain-of-thought)
5. [实战 Prompt 模板库](#5-实战-prompt-模板库)
6. [Prompt 调试技巧](#6-prompt-调试技巧)
7. [API 调用的 Prompt 管理](#7-api-调用的-prompt-管理)
8. [核心要点回顾](#8-核心要点回顾)
9. [参考资料](#9-参考资料)
...
| **CoT（思维链）** | 要求模型展示推理步骤 | 数学、逻辑推理、代码调试 | 较高 |

> **重点**：写好 Prompt 是使用大模型成本最低、回报最高的技能，无需微调就能让输出质量大幅提升。
...
> **重点**：动态示例选择显著优于固定示例，通过向量相似度匹配与输入最相关的示例，可大幅提升生成质量。
...
debug_examples = [
    ("程序报 NullPointerException at UserService.java:25",
     ...
     "\n答案：建议添加 null 检查，或使用 Optional.ofNullable(user).map(User::getName)。"),
]

prompt = few_shot_cot_prompt(
    "启动 Spring Boot 时报错 'Failed to configure a DataSource: url attribute is not specified'",
    debug_examples
)
```
...
- **从 Zero-Shot 开始**：大部分任务 Zero-Shot 加清晰指令即可满足需求
- **遇到瓶颈再加 Few-Shot**：2-3 个精心挑选的示例胜过 10 个随便选的
- **推理任务必须加 CoT**：数学、逻辑、Bug 定位场景，CoT 效果提升显著
- **动态示例选择**（RAG + Few-Shot）通过向量相似度匹配最优示例，大幅提升质量
- **Self-Consistency** 多次采样投票取优，适合高准确性场景
- **把 Prompt 当代码管理**：版本控制、测试用例、团队 Review
...
6. LangChain Prompt Template 文档
