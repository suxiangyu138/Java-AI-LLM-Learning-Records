...
> **核心摘要**：Skill 是介于 Tool（原子操作）和 Agent（自主系统）之间的中间层，包含工作流、领域知识和工具组合。本文详细阐述 Skill 的概念定位、核心组成、两阶段加载机制、设计原则以及开发实战方法。
...
一个完整的 Skill 包含以下五个要素：

| 要素 | 说明 |
|---|---|
| **SKILL.md** | Skill 定义：名称、描述、触发条件、工作流 |
| **Prompt Template** | 专用 System Prompt |
| **Tools** | 该 Skill 专用的工具 |
| **Knowledge** | 领域知识（规则、最佳实践） |
| **Output Schema** | 标准化输出格式 |
...
| 方式 | Token 消耗 | 问题 |
|---|---|---|
| 一次性加载所有 Skill | 100+ Skills × 5KB = 500KB+ | 浪费 Token，无关 Skill 干扰决策 |
| **两阶段加载** | 先扫描 1KB，再按需加载 5KB | Token 节省约 100 倍 |
...
| **可组合** | Skill A 的输出是 Skill B 的输入 | 紧耦合成一个大 Skill | 代码审查 → 问题修复，两个独立 Skill |
...
| 适合做成 Skill | 留给 Agent 自行处理 |
|---|---|
| 可重复执行的工作流 | 一次性任务 |
| 有明确的输入和输出 | 需要完全自主决策 |
| 依赖领域知识而非通用能力 | 通用能力（写代码、回答问题） |
| 值得为它写一份专用 Prompt | — |
...
- Tool 是原子操作，Skill 是工作流 + 知识 + 工具组合，Agent 是自主系统
- 两阶段加载：先扫描 description（省 Token），再按需加载完整 SKILL.md
- SKILL.md 包含：名称、描述、触发条件、工作流、工具、输出格式
- 核心设计原则：单一专注、好描述、结构化输出、有边界

---

## 参考资料

1. LangChain 官方文档. Tool 与 Agent 设计指南
2. Anthropic 官方文档. Effective Tool Use 最佳实践
3. OpenAI 官方文档. Function Calling 架构设计
