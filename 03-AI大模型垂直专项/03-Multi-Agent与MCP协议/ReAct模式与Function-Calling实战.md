...
> **核心摘要**：ReAct 和 Function Calling 是实现 Agent 自主行动的两大基础范式。ReAct 通过 Prompt 驱动"思考-行动-观察"循环，Function Calling 通过 API 原生支持结构化工具调用。本文从零实现 ReAct Agent、基于 OpenAI 协议的 Function Calling 流程、LangChain 集成方案到智能运维 Agent 实战，提供完整的代码示例。
...
Final Answer: 北京今天天气很好！晴天，18-25℃，空气质量优，非常适合户外运动。
```
...
## 三、Function Calling（OpenAI 兼容协议）
...
class DevOpsAgent:
    ...
    def _build_tools(self) -> dict:
        return {
            ...
            "check_system": lambda: self._system_health_check(),
            "search_solution": lambda error: self._search_solution_db(error),
        }
...
- ReAct 适合需要自由思考的多步任务，Function Calling 适合标准的工具调用
- 从零实现 ReAct Agent 只需解析 Thought / Action / Observation 格式的文本
- Function Calling 的核心流程：定义工具 → LLM 决定调用 → 执行工具 → 回传结果
- ToolRegistry 模式可统一管理所有工具的定义、注册和执行
- 推荐组合：Function Calling 做基础设施 + ReAct 引导复杂推理

---

## 参考资料

1. LangChain 官方文档. Agent 与 Tool 开发指南
2. OpenAI 官方文档. Function Calling API 参考
3. DeepSeek 官方文档. 兼容 OpenAI 的 API 调用
4. Python re 模块官方文档. 正则表达式解析
