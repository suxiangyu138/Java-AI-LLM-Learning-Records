...
**Function Calling 的生产级三件套**（2026 共识）：

```text
① 超时必设：每个工具调用有超时（默认 10-30s）
   → 无超时的工具调用 = 线程/请求泄漏的温床
② 重试策略：网络类错误可重试（指数退避），业务拒绝不重试
   → 幂等性：重试前确认工具幂等（或加幂等键）
③ 失败降级：工具失败 → Agent 换工具/换策略/明确告知用户
   → 不允许"静默失败"（假装成功是最坏的结果）
```
...
| 失败 | 表现 | 修复 |
|------|------|------|
| 选错工具 | 调了"搜索"而不是"查数据库" | 优化工具 description（见 04 号） |
| 参数错配 | JSON Schema 校验失败 | 参数 Schema 示例补全 |
| 幻觉参数 | 编造不存在的 ID | 工具先查存在性再操作 |
| 描述注入 | 工具描述被恶意改写 | **固定（pin）工具描述**（2026 安全要点） |

> 🎯 **核心要点**：Function Calling 的质量 = "**选得对（描述）+ 调得稳（超时/重试）+ 败得明（降级）**"三件事——2026 年评估指标 `FunctionCallAccuracy` 与 `ToolSelectionAccuracy` 就是这三件事的度量（优化器可自动改进工具描述，提升 10-25 个百分点）。
