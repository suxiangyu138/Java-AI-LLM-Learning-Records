...
| 模式 | 适用问题 | 复杂度 | 典型框架 |
|------|---------|:------:|---------|
| 单 Agent + 工具 | 单域任务（客服/搜索） | 低 | LangChain/LangChain4j |
| ReAct 循环 | 需要推理-行动交替 | 中 | 通用（见 03 号） |
| Plan-Execute | 任务可预分解 | 中高 | LlamaIndex |
| **多 Agent 协作** | 跨域复杂任务 | **高** | LangGraph/AutoGen（见 07 号） |
...
├── 是 → 单 Agent + 工具（最小可行）
└── 否 → 需要分工协作？
    ├── 是 → 多 Agent（先评估：能否拆成独立子任务）
    └── 否 → 复杂单 Agent（Plan-Execute/ReAct）
再问：状态复杂吗？→ 是 → LangGraph（图状态机）
     需要人类审批？→ 是 → 加审批门（见 10 号安全）
```

> 🎯 **核心要点**：设计模式选择 = "**任务域 × 协作需求 × 状态复杂度**"三维决策——**先单后多**（能单 Agent 解决的不要上多 Agent，复杂度爆炸是 2026 最常见的过度设计）。
