...
**向量库在 RAG 中的角色升级（2026）**：

```text
传统角色：Top-K 相似检索（文本块）
2026 新增角色：
  ① 混合检索承载：BM25 稀疏 + 稠密双路（库内实现，RRF 融合）
     → "混合检索是刚需"（精确 SKU/版本号/术语查询）
  ② 多模态向量：图文统一检索（ColPali 等视觉向量，见 RAG 深化 19 号）
  ③ Agent 记忆：长期记忆的向量存储（见 Multi-Agent 05 号）
  ④ 元数据过滤增强：权限/租户/时间的向量+过滤联合查询
```

**2026 RAG 向量层的架构决策**：

```text
① 规模 <5000 万 + 已有 Postgres → pgvector（零新增）
② 混合检索刚需 → Weaviate/Qdrant（原生 BM25+RRF）
③ 亿级/高 QPS → Milvus（云原生 + 磁盘索引）
④ 快速验证 → Chroma/Pinecone
⑤ 关键：过滤能力评估（租户隔离场景）——"先搜后滤"会伤害召回

与 RAG 深化的衔接：查询改写/重排/上下文压缩（02-RAG 深化 13-15 号）
→ 向量库负责"召回"环节，重排在其后（两者是上下游）
```

> 🎯 **核心要点**：2026 向量库在 RAG 中的角色 = "**混合检索 + 过滤 + 多模态 + 记忆**"四合一——选型时**过滤能力与混合检索是新增的必考维度**（纯 Top-K 时代已过去）。
...
① 召回：Top-K 语义检索（主角色）
② 过滤：元数据/权限/租户（与检索联合）
③ 混合：BM25 稀疏 + 稠密 + RRF（2026 刚需）
④ 记忆：Agent 长期记忆存储（Multi-Agent 05 号）
⑤ 多模态：图文统一检索（ColPali，RAG 深化 19 号）

典型链路：
  文档 → 切块 → Embedding → 入库（元数据）
  查询 → Embedding → 混合检索（+过滤）→ 重排 → 上下文 → LLM
```
...
| 环节 | 用什么 | 查哪里 |
|------|--------|--------|
| 向量库深度（Milvus） | Milvus 专题 | `03-AI大模型应用开发/Milvus/` |
| 查询改写/重排/压缩 | RAG 深化 | `02-RAG检索增强生成/13-15` |
| 检索评估（RAGAS） | RAG 深化 | `02-RAG检索增强生成/20` |
| Agent 记忆 | Multi-Agent | `03-Multi-Agent与MCP协议/05` |

> 🎯 **核心要点**：向量库是 RAG 的"召回引擎"但不是全部——**上游（切分/Embedding）、下游（重排/生成）、侧翼（评估/记忆）**共同构成 RAG 系统；2026 选型时把"混合检索 + 过滤 + 评估"一起纳入考量。
