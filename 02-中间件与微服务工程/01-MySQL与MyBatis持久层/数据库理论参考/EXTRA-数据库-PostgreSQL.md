# 数据库系统概念（PostgreSQL）—— Java 后端开发视角深度剖析

> PostgreSQL（简称 PG）是一款开源、强大、企业级关系型数据库，以其高度标准兼容、强大扩展性、丰富数据类型与复杂查询能力著称，是 Java 后端开发中 MySQL 的重要替代方案。

## 📑 目录

- [一、PostgreSQL 概述与核心定位](#一postgresql-概述与核心定位)
- [二、PostgreSQL 核心理论](#二postgresql-核心理论)
- [三、PostgreSQL 数据类型](#三postgresql-数据类型)
- [四、PostgreSQL 高级特性](#四postgresql-高级特性)
- [五、Java 后端集成 PostgreSQL](#五java-后端集成-postgresql)
- [六、PostgreSQL 性能优化](#六postgresql-性能优化)
- [七、PostgreSQL 高可用与集群](#七postgresql-高可用与集群)
- [八、PostgreSQL 扩展生态](#八postgresql-扩展生态)
- [九、PostgreSQL 与 MySQL 对比](#九postgresql-与-mysql-对比)
- [十、企业级最佳实践](#十企业级最佳实践)
- [十一、总结](#十一总结)
...
| 特性 | 说明 |
|------|------|
| **SQL 标准兼容** | 完全遵循 SQL 标准，支持复杂 SQL、窗口函数、CTE、递归查询 |
| **丰富数据类型** | JSON、JSONB、数组、hstore、几何类型、枚举、范围类型 |
| **强大索引体系** | B-tree、GiST、GIN、BRIN、SP-GiST、Hash |
| **事务与约束** | 支持事务、外键、视图、触发器、存储过程、函数、规则 |
| **并发控制** | 行级锁、MVCC、多版本快照 |
| **高可用** | 热备、流复制、逻辑复制 |
| **扩展能力** | PostGIS、pg_stat_statements、pg_cron、TimescaleDB 等 |
| **企业级特性** | MVCC、在线 DDL、并行查询 |
...
PostgreSQL 使用 MVCC 实现高并发读写，避免读写阻塞：
...
> **Java 后端实践**：使用 `@Transactional` 控制事务；合理设置隔离级别（默认读已提交）；避免长事务导致膨胀与性能问题。
...
| 索引类型 | 适用场景 |
|---------|---------|
| **B-tree** | 默认索引，适合等值、范围查询 |
| **GIN** | 适合 JSONB、数组、全文检索 |
| **GiST** | 适合空间数据、范围类型、几何数据 |
| **BRIN** | 适合时序数据、大表、连续数据 |
| **Hash** | 适合等值查询 |
| **SP-GiST** | 适合非平衡数据（如 IP、文本） |

> **Java 后端优化**：字符串查询用 B-tree；JSON 字段用 GIN；空间数据用 GiST（PostGIS）；时序数据用 BRIN 或 TimescaleDB。
...
| 方案 | 说明 |
|------|------|
| **主从复制（流复制）** | 主库写入，从库复制，支持同步/异步复制 |
| **Patroni + etcd** | 自动故障切换，适合生产环境 |
| **读写分离** | 使用 MyBatis 动态数据源或 ShardingSphere |
| **分库分表** | 使用 ShardingSphere 适配 PostgreSQL |
