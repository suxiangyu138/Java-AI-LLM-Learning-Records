...
> **数据库系统概念中最经典、最完整、最贴近企业级设计的教学案例。本文以大学教务管理系统为背景，覆盖实体建模、关系设计、范式理论、事务处理、查询优化、索引、约束、视图、存储过程、触发器、安全、高可用等全部核心知识点，从 Java 后端开发视角进行深度剖析。**

---

## 📑 目录

1. [大学模式（University Schema）概述](#一大学模式university-schema概述)
2. [大学模式核心需求分析](#二大学模式核心需求分析)
3. [概念数据模型（E-R 图设计）](#三概念数据模型e-r-图设计)
4. [逻辑数据模型（关系模式设计）](#四逻辑数据模型关系模式设计)
5. [物理数据模型（MySQL 建表语句）](#五物理数据模型mysql-建表语句)
6. [Java 后端核心业务实现](#六java-后端核心业务实现)
7. [数据库高级特性应用](#七数据库高级特性应用)
8. [性能优化与高可用设计](#八性能优化与高可用设计)
9. [安全与权限控制](#九安全与权限控制)
10. [大学模式对 Java 后端的核心价值](#十大学模式对-java-后端的核心价值)
11. [总结](#十一总结)
...
大学模式（University Schema）是数据库系统概念教材中最经典的教学案例，它以大学教务管理系统为背景，包含学生、教师、课程、选课、院系、教室、时间段等实体。
...
| 模块 | 功能描述 |
|------|---------|
| 学生管理 | 注册、信息维护、成绩查询、学籍状态 |
| 教师管理 | 信息管理、授课安排、成绩录入 |
| 课程管理 | 课程信息、学分、先修课、课程类型 |
| 选课管理 | 选课、退课、冲突检查、人数限制 |
| 院系管理 | 院系信息、专业设置、负责人 |
| 教室管理 | 教室容量、设备、使用时间 |
| 排课管理 | 课程时间、教室分配、教师时间冲突检查 |
| 成绩管理 | 成绩录入、审核、统计、绩点计算 |
| 权限管理 | 学生、教师、管理员不同操作权限 |

### 2.2 核心数据特征

| 特性 | 指标 |
|------|------|
| 数据量 | 学生 10000+、课程 1000+、选课记录 100000+ |
| 并发特征 | 选课高峰期 QPS 1000+，需防超选、防冲突 |
| 一致性要求 | 选课、成绩录入必须强一致 |
| 实时性要求 | 选课结果、成绩需实时可见 |
| 可扩展性 | 支持多校区、多学院、分库分表 |
...
| 实体 | 主键 | 主要属性 |
|------|------|---------|
| 学生（student） | 学号 | 姓名、性别、出生日期、院系号、入学时间、状态 |
| 教师（instructor） | 工号 | 姓名、性别、院系号、职称、入职时间、工资 |
| 课程（course） | 课程号 | 课程名、学分、学时、院系号、先修课号、类型 |
| 选课（takes） | 学号 + 课程号 | 成绩、选课时间 |
| 院系（department） | 院系号 | 院系名、负责人工号、位置、电话 |
| 教室（classroom） | 教室号 | 容量、楼号、设备类型 |
| 时间段（time_slot） | 时间段编号 | 星期、节次、开始时间、结束时间 |
| 授课（teaches） | 工号 + 课程号 + 教室号 + 时间段编号 | — |

### 3.2 实体间关系

| 关系 | 类型 | 说明 |
|------|------|------|
| 学生 ↔ 选课 ↔ 课程 | 多对多 | 一个学生选多门课，一门课被多学生选 |
| 教师 ↔ 授课 ↔ 课程 | 多对多 | 一个教师教多门课，一门课被多教师教 |
| 院系 ↔ 学生 | 一对多 | 一个院系多个学生 |
| 院系 ↔ 教师 | 一对多 | 一个院系多个教师 |
| 院系 ↔ 课程 | 一对多 | 一个院系多个课程 |
| 授课 ↔ 教室 | 多对一 | 多个授课用同一教室 |
| 授课 ↔ 时间段 | 多对一 | 多个授课用同一时间段 |
| 课程 ↔ 先修课 | 自关联 | 一门课有多个先修课 |

---

## 四、逻辑数据模型（关系模式设计）

> 基于 E-R 模型转换为关系模式，满足 3NF，消除冗余，保证数据一致性。

| 序号 | 关系模式 | 说明 |
|------|---------|------|
| 1 | student(s_id, s_name, gender, birth, dept_id, enroll_time, status) | 学生表 |
| 2 | instructor(i_id, i_name, gender, dept_id, title, hire_time, salary) | 教师表 |
| 3 | course(c_id, c_name, credit, hour, dept_id, pre_c_id, type) | 课程表 |
| 4 | takes(s_id, c_id, score, take_time) | 选课表 |
| 5 | department(dept_id, dept_name, leader_id, location, phone) | 院系表 |
| 6 | classroom(r_id, capacity, building, equipment) | 教室表 |
| 7 | time_slot(t_id, week, section, start_time, end_time) | 时间段表 |
| 8 | teaches(i_id, c_id, r_id, t_id) | 授课表 |
...
public class TakeServiceImpl implements TakeService {
    ...
    public void selectCourse(String sId, String cId) {
        ...
        // 2. 检查是否已选该课程
        Take take = takeMapper.selectOne(
            new QueryWrapper<Take>().eq("s_id", sId).eq("c_id", cId));
        if (take != null) {
            throw new RuntimeException("已选该课程");
        }
        // 3. 检查先修课是否完成（如有）
        if (course.getPreCId() != null) {
            Take preTake = takeMapper.selectOne(
                new QueryWrapper<Take>()
        ...
        List<Teaches> teachesList = teachesMapper.selectList(
            new QueryWrapper<Teaches>().eq("c_id", cId));
...
public class ScoreServiceImpl implements ScoreService {
    ...
    public BigDecimal calculateGPA(String sId) {
        List<Take> takes = takeMapper.selectList(
            new QueryWrapper<Take>().eq("s_id", sId).isNotNull("score"));
...
public List<ScoreVO> getScoreList(String sId) {
    // 1. 查询选课记录
    List<Take> takes = takeMapper.selectList(
        new QueryWrapper<Take>().eq("s_id", sId));
    if (CollectionUtils.isEmpty(takes)) {
...
> **主库**写入（选课、成绩录入），**从库**读取（成绩查询、课程查询）。

### 8.3 缓存设计

| 缓存对象 | 缓存技术 | 说明 |
|---------|---------|------|
| 热门课程信息 | Redis | 减轻数据库查询压力 |
| 学生基本信息 | Caffeine | 本地缓存，低延迟 |
| 选课结果 | Redis | 减轻数据库写入压力 |
