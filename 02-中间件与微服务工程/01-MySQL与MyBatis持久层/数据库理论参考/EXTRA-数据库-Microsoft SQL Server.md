# 数据库系统概念（Microsoft SQL Server）—— Java 后端开发视角深度详细剖析

> Microsoft SQL Server（简称 SQL Server）是微软推出的企业级关系型数据库管理系统（RDBMS），以易用性、稳定性、安全性、与 Windows 生态深度融合、强大的商业智能与分析能力著称。广泛应用于金融、政务、制造、零售、医疗等行业。

## 📑 目录

- [一、SQL Server 概述与企业级定位](#一sql-server-概述与企业级定位)
- [二、SQL Server 核心理论（Java 后端视角）](#二sql-server-核心理论java-后端视角)
  - [（一）体系结构](#一体系结构)
  - [（二）事务与 ACID 实现](#二事务与-acid-实现)
  - [（三）锁机制与并发控制](#三锁机制与并发控制)
  - [（四）事务日志](#四事务日志)
  - [（五）索引体系](#五索引体系)
- [三、SQL Server 数据类型（Java 后端映射）](#三sql-server-数据类型java-后端映射)
- [四、T-SQL 方言与高级特性（Java 后端核心）](#四t-sql-方言与高级特性java-后端核心)
- [五、Java 后端集成 SQL Server（实战）](#五java-后端集成-sql-server实战)
- [六、SQL Server 性能优化（Java 后端必备）](#六sql-server-性能优化java-后端必备)
- [七、SQL Server 高可用与灾备](#七sql-server-高可用与灾备)
- [八、SQL Server 安全特性](#八sql-server-安全特性)
- [九、SQL Server 与其他数据库对比](#九sql-server-与其他数据库对比)
- [十、企业级最佳实践（Java 后端）](#十企业级最佳实践java-后端)
- [十一、总结](#十一总结)
...
SQL Server 不仅支持标准 SQL，还提供 **T-SQL（Transact-SQL）方言**、丰富的数据类型、高级索引、事务、存储过程、触发器、复制、高可用、安全审计等企业级特性。

> 近年来，SQL Server 已全面支持 Linux、Docker、Kubernetes，成为跨平台数据库，与 Java 后端生态的兼容性大幅提升，是构建高可靠、高性能、易维护、合规企业级系统的重要选择。
...
SQL Server 采用 **实例（Instance）+ 数据库（Database）+ 文件组（Filegroup）+ 数据文件** 的架构：

| 层级 | 说明 |
|------|------|
| 实例（Instance） | SQL Server 运行环境，包含内存结构（Buffer Pool、Plan Cache）与后台进程 |
| 数据库（Database） | 逻辑容器，包含表、索引、视图、存储过程等 |
| 文件组（Filegroup） | 管理数据文件，提升 I/O 性能 |
...
| 读已提交（Read Committed） | **默认级别**，防止脏读 |
| 可重复读（Repeatable Read） | 防止脏读、不可重复读 |
| 可串行化（Serializable） | 最高级别，防止所有并发问题 |
| 快照隔离（Snapshot） | 基于行版本控制，无锁读 |
| 读已提交快照（RCSI） | 改进版读已提交 |
...
| 锁类型 | 说明 |
|--------|------|
| 行锁（RID、Key Lock） | 锁定单行 |
| 页锁（Page Lock） | 锁定数据页 |
| 表锁（Table Lock） | 锁定整表 |
| 意向锁（Intent Lock） | 提升并发性能 |
| 行版本控制（RVC） | 实现无锁读 |
...
| SQL Server 类型 | 说明 | Java 映射 |
|----------------|------|----------|
| `VARCHAR(n)` | 可变长度 | `String` |
| `NVARCHAR(n)` | Unicode 可变长度（**推荐中文**） | `String` |
| `CHAR(n)` | 固定长度 | `String` |
| `VARCHAR(MAX)` | 大文本（推荐替代 TEXT） | `String` |

### （二）数值类型

| SQL Server 类型 | 说明 | Java 映射 |
|----------------|------|----------|
| `INT` / `BIGINT` / `SMALLINT` | 整数 | `Integer` / `Long` |
| `DECIMAL(p,s)` | 高精度小数（**金融必用**） | `BigDecimal` |
| `FLOAT` / `REAL` | 浮点数 | `Double` / `Float` |
| `MONEY` / `SMALLMONEY` | 货币类型 | `BigDecimal` |
...
| `DATE` | 日期 | `LocalDate` |
| `TIME` | 时间 | `LocalTime` |
| `DATETIME` | 日期+时间 | `LocalDateTime` |
| `DATETIME2` | 高精度日期时间（**推荐**） | `LocalDateTime` |
| `DATETIMEOFFSET` | 带时区 | `OffsetDateTime` |
...
| 方案 | 说明 |
|------|------|
| **Always On 可用性组** | 主从复制，自动故障切换，支持多副本 |
| **数据库镜像** | 传统高可用方案 |
| **日志传送** | 低成本灾备方案 |
| **备份与恢复** | 完整备份、差异备份、日志备份、时间点恢复 |
