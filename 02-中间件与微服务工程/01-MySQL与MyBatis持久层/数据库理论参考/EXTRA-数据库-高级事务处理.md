# 数据库系统概念（高级事务处理）—— Java后端开发视角深度剖析

## 📑 目录

- [一、高级事务处理核心内容（原书框架）](#一高级事务处理核心内容原书框架)
- [二、Java后端开发视角深度剖析](#二java后端开发视角深度剖析)
  - [（一）事务模型扩展](#一事务模型扩展)
  - [（二）分布式事务：Java微服务的核心痛点](#二分布式事务java微服务的核心痛点)
  - [（三）事务并发控制进阶：高并发性能关键](#三事务并发控制进阶高并发性能关键)
  - [（四）事务恢复与容错：系统可靠性保障](#四事务恢复与容错系统可靠性保障)
  - [（五）实时事务：低延迟业务支撑](#五实时事务低延迟业务支撑)
  - [（六）事务与中间件集成](#六事务与中间件集成)
  - [（七）云原生与微服务事务](#七云原生与微服务事务)
- [三、高级事务处理对Java后端核心价值](#三高级事务处理对java后端核心价值)
...
高级事务处理是数据库系统在基础事务（ACID、隔离级别、封锁）之上，面向复杂业务、分布式环境、高并发场景的延伸能力。其核心内容包括：

| 核心模块 | 主要内容 | 说明 |
|---------|---------|------|
| 事务模型扩展 | 嵌套事务、链式事务、长事务、柔性事务 | 复杂业务的事务组织方式 |
| 分布式事务 | 2PC、3PC、TCC、SAGA、本地消息表 | 跨服务数据一致性 |
| 并发控制进阶 | MVCC、锁优化、乐观锁/悲观锁、死锁 | 高并发性能关键 |
| 事务恢复与容错 | 事务日志、检查点、故障恢复、事务补偿 | 系统可靠性保障 |
| 实时事务 | 截止时间、优先级调度、软/硬实时 | 低延迟业务支撑 |
| 中间件集成 | 消息队列、缓存、分库分表事务 | 复杂场景适配 |
| 云原生事务 | 分布式事务协调、无侵入事务、跨服务事务 | 现代架构趋势 |
...
传统事务不适合持续数分钟到数天的业务（如审批流程、合同签署），通过以下方式解决：
- 拆分事务
- 状态机 + 补偿机制
- SAGA模式
...
不同于ACID刚性事务，柔性事务追求 **基本可用（BA）**、**柔性状态（S）**、**最终一致性（E）**，是微服务主流选择。
...
| 表锁 | 粗粒度锁定 | 低并发、批量操作 |
| 意向锁 | 提升表锁效率 | 表级与行级协同 |
| 间隙锁（Next-Key Lock） | 防止幻读 | InnoDB RR级别 |
...
| 日志类型 | 作用 | 说明 |
|---------|------|------|
| Redo Log | 保证持久性 | 事务提交必须等待Redo Log落盘 |
| Undo Log | 保证原子性、MVCC | 回滚依赖Undo Log |
...
| 方案 | 说明 | 实现 |
|------|------|------|
| 无侵入事务 | 自动生成undo_log，无需修改业务代码 | Seata AT模式 |
| 多数据源事务 | 管理跨库事务 | Spring + Atomikos |
| 服务网格事务 | 分布式事务协调 | Istio（未来趋势） |
