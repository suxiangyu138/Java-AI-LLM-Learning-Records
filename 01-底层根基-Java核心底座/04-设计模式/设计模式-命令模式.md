...
| 对比维度 | 命令模式 | 策略模式 |
| --- | --- | --- |
| 核心目的 | 封装请求，解耦发送者与执行者 | 封装算法，替换不同实现方案 |
| 关注重点 | 请求的发起、传递、执行、撤销 | 算法的选择、替换、执行 |
| 角色关系 | 调用者→命令→接收者（三层） | 上下文→策略（两层） |
| 可扩展性 | 新增命令，不影响调用者 | 新增策略，替换原有算法 |
| 核心场景 | 操作排队、撤销、日志、异步 | 支付方式、排序算法、业务规则 |
