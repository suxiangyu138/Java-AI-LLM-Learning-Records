...
对象属性数量多、存在必选/可选参数、参数之间有依赖关系、需要保证对象创建完成后不可变、避免零散set赋值。
...
后端实战中，为了简化代码，常把抽象建造者+指挥者合并，直接在产品类内部定义静态内部建造者类，实现链式调用，也就是日常最常用的**链式建造者**。
...
| 后端典型场景 | DTO、配置类、不可变Bean | 多数据源、多中间件适配 |
