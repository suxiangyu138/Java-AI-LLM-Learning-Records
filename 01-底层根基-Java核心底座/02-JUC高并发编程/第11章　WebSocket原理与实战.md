...
1. 初始化主从Reactor线程组，配置ServerBootstrap，绑定端口，与前文Netty服务端启动流程一致；
2. 在ChannelPipeline中按顺序装配HTTP编解码器、HTTP消息聚合器、分块写入处理器、WebSocket协议处理器、自定义业务处理器；
3. 自定义业务处理器专注处理文本/二进制业务数据帧，无需关注握手、心跳、编解码等底层逻辑，实现业务与底层解耦。
...
<head>
    <meta charset="UTF-8">
    <title>Netty WebSocket实时测试</title>
</head>
<body>
    <h2>Netty WebSocket实时通信</h2>
    <div>
        <input id="messageInput" type="text" placeholder="输入消息" />
        <button onclick="sendMessage()">发送消息</button>
    </div>
    <div id="messageArea" style="margin-top: 20px; border: 1px solid #ccc; height: 300px; overflow-y: auto; padding: 10px;"></div>
    <script type="text/javascript">
        // 创建WebSocket连接
        var ws = new WebSocket("ws://localhost:8088/ws");
        // 连接成功回调
        ws.onopen = function() {
            appendMessage("[系统] 连接成功，欢迎进入Netty WebSocket实时通信！");
        };
        // 接收消息回调
...
1. 启动NettyWebSocketServer服务，控制台打印启动成功信息，确认监听8088端口；
2. 打开前端HTML测试页面，浏览器自动与服务端建立WebSocket连接，页面提示连接成功；
3. 输入消息点击发送，服务端接收消息并广播至所有在线客户端，多开页面测试实时广播功能；
4. 关闭测试页面，控制台打印客户端离线信息，确认在线人数更新；
5. 模拟网络中断，测试连接异常处理，确认服务端自动清理无效连接。
