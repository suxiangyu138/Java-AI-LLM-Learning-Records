# 数据结构与算法分析：图论算法（Java 后端开发视角深度剖析）

## 📑 目录

- [一、图论核心概念](#一图论核心概念后端开发必备)
- [二、图的遍历算法](#二图的遍历算法基础核心)
- [三、最短路径算法](#三最短路径算法后端高频)
- [四、最小生成树（MST）算法](#四最小生成树mst算法后端高频)
- [五、拓扑排序](#五拓扑排序有向无环图)
- [六、强连通分量（SCC）算法](#六强连通分量scc算法)
- [七、Java 后端面试高频题](#七java-后端面试高频题)
- [八、总结](#八总结java-后端核心要点)
...
| 存储结构 | 说明 | 适用场景 | 查询边 | 空间复杂度 |
| :--- | :--- | :--- | :--- | :--- |
| **邻接矩阵** | 二维数组 | 稠密图 | O(1) | O(V²) |
| **邻接表** | 数组 + 链表 / 列表 | 稀疏图 | O(度) | O(V + E) |
...
> **注意**：Dijkstra 算法要求图中不能有负权边，否则贪心策略失效。

### 2. Bellman-Ford 算法（单源最短路径，含负权）

**核心原理**

动态规划，松弛 V - 1 轮，第 V 轮检测负权环。
...
- 邻接矩阵与邻接表的区别与适用场景？
- DFS 与 BFS 的原理、时间复杂度、应用场景？
- Dijkstra 与 Bellman-Ford 的区别？
...
2. **遍历**：DFS（深度探索）、BFS（最短路径），时间 O(V + E)
3. **最短路径**：Dijkstra（非负权）、Bellman-Ford（负权）、Floyd（多源）
4. **最小生成树**：Kruskal（稀疏图）、Prim（稠密图）
5. **拓扑排序**：Kahn 算法，用于依赖调度
6. **强连通分量**：Tarjan 算法，用于环检测、社区发现
7. **后端高频场景**：网络路由、任务调度、依赖解析、社交网络、推荐系统

> 图论算法是 Java 后端开发的重要理论基础，掌握其工程化实现能够有效解决复杂关系分析、路径规划、连通性判断等实际问题。
