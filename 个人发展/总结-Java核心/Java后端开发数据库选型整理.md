...
结合Java后端开发的实际场景，把能用到的数据库按主流核心、小众补充、特殊场景三类整理，同时标注适用场景、Java整合方式（主流框架），方便直接选型。也补充了国产数据库（国产替代趋势）和海外常用库，覆盖全栈需求。
...
**一句话总结：** Java后端最核心的是MySQL + Redis，进阶加PostgreSQL/MongoDB/ES，国产化选达梦/金仓/OceanBase，特殊场景用ClickHouse/Neo4j。
