...
ZooKeeper集群遵循过半存活原则，只要集群中超过半数节点正常运行，整个集群就可用，因此生产环境通常部署奇数个节点（3台、5台），既保证高可用，又节省资源。
...
Watch机制具备一次性触发特性，一次通知触发后，监听会自动失效，如需持续感知变化，需要重新注册监听，Netty整合ZooKeeper时会封装该逻辑，实现持续监听。
...
13.5 集群测试与验证
13.5.1 测试流程
启动ZooKeeper单机服务，确认端口2181正常监听；
启动第一个DistributedNettyHttpServer节点（8080端口），控制台打印服务注册成功日志，查看ZooKeeper节点创建成功；
修改端口为8081，启动第二个服务节点，实现双节点集群；
启动ServerDiscoverClient客户端，打印初始两个可用节点；
关闭8080端口节点，观察客户端实时感知节点移除，打印可用节点仅剩8081；
重新启动8080端口节点，客户端实时感知节点新增，恢复双节点列表；
访问任意节点，验证HTTP服务正常运行，业务逻辑不受集群影响。
13.5.2 常见问题排查
